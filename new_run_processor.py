...
def main_processor(run_date_str):
    ...
    while True:
        ...
        print(f"Status set to 'running'. Executing 'new_ai_personCount.py'...")
        ...
        try:
            ...
            print(f"--- 🚀 Starting subprocess for {task_camera_name} ---")

            my_env = os.environ.copy()
            my_env["PYTHONUNBUFFERED"] = "1"
            
            # --- ( ✨ 1. แก้ไขส่วนนี้: รันและอ่าน Log แบบ Real-time ✨ ) ---
            process = subprocess.Popen(command, 
                                       stdout=subprocess.PIPE, 
                                       stderr=subprocess.STDOUT, # รวม stdout และ stderr มาที่ช่องทางเดียว
                                       text=True, 
                                       encoding='utf-8',
                                       bufsize=1,
                                       env=my_env) # ตั้งค่าเป็น Line-buffered (อ่านทีละบรรทัด)

            # วนลูปอ่าน output จาก ai_personCount.py ทีละบรรทัด
            if process.stdout:
                for line in iter(process.stdout.readline, ''):
                    # พิมพ์ log ที่ได้จากไฟล์ลูกทันที
                    # คุณจะเห็น "Loading AI model...", "Processing started..." ฯลฯ ที่นี่
                    print(f"    [AI_LOG] {line.strip()}")
            
            process.wait() # รอให้ process จบจริงๆ
            print(f"--- ✅ Subprocess for {task_camera_name} finished (Return Code: {process.returncode}) ---")
            # --- ( สิ้นสุดการแก้ไข ) ---

            if process.returncode == 0:
                print(f"Successfully processed '{task_camera_name}'.")
                # (ไม่จำเป็นต้องพิมพ์ stdout อีก เพราะเราพิมพ์ไปแล้ว)
                new_status = 'completed'
            else:
                print(f"!!! FAILED to process '{task_camera_name}' !!!")
                # (Error log ถูกพิมพ์ไปแล้วแบบ real-time)
                new_status = 'failed'
