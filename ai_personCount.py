...
def make_side_label(a, b):
    ...
    below = (mid[0], mid[1] + 100)
    return _cross_sign(below, a, b) < 0

def is_top_to_bottom(p1, p2, a, b, neg_is_bottom):
    if not is_crossing_line(p1, p2, a, b, allow_touch=True):
        return False
    s1 = _cross_sign(p1, a, b)
    s2 = _cross_sign(p2, a, b)
    if s1 == 0 or s2 == 0:
...
def _on_mouse(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_MOUSEMOVE:
        mouse_pos_disp = (x, y)
        rx = int(x * W / display_width)
        ry = int(y * H / display_height)
        mouse_pos_raw = (rx, ry)
...
def draw_lines(img):
    cv2.line(img, red_line[0], red_line[1], (0, 0, 255), 2)
    cv2.line(img, blue_line[0], blue_line[1], (255, 0, 0), 2)
    cv2.line(img, green_line[0], green_line[1], (0, 255, 0), 2)
    cv2.line(img, yellow_line[0], yellow_line[1], (0, 255, 255), 2)

def put_text(img, text, xy, color=(255,255,255)):
    cv2.putText(img, text, xy, cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2, cv2.LINE_AA)
